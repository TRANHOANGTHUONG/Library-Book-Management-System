...
```bash
mysql -u root -p < quanlythuvien-2_7_26.sql ----- Có chạy file sql rồi thì không cần chạy lệnh này oke?
```

## 3. Cấu hình kết nối

Mặc định `thuvien_django/settings.py`: HOST=127.0.0.1, PORT=3306,
NAME=QuanLyThuVien, USER=root, PASSWORD="" (Tự đánh pass dô đi, hoặc set cái biến môi trường note ở dưới oke?).
